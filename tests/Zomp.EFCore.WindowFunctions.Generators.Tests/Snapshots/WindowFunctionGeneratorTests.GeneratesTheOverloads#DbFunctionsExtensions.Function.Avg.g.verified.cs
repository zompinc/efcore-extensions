...
public static partial class DbFunctionsExtensions
{
    ...
    /// <returns>Avg for the selected window frame.</returns>
    ...
    public static T? Avg<T>(this DbFunctions _, T expression, OverClause over)
        where T : struct
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(Avg))  + UseWindowFunctions);
    ...
    /// <returns>Avg for the selected window frame.</returns>
    /// <exception cref="InvalidOperationException">Occurs on client-side evaluation.</exception>
    public static TResult? Avg<T, TResult>(this DbFunctions _, T expression, OverClause over)
        where T : struct
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(Avg))  + UseWindowFunctions);

...
{
    /// <summary>
    /// The DENSE_RANK() window function returns the dense rank value of the expression across all input values.
    /// </summary>
    /// <param name="_">The <see cref="DbFunctions"/> instance.</param>

    /// <param name="over">over clause.</param>
    /// <returns>DenseRank for the selected window frame.</returns>
    ...
    public static long DenseRank(this DbFunctions _, OverClause over)
        => throw new InvalidOperationException(CoreStrings.FunctionOnClient(nameof(DenseRank))  + UseWindowFunctions);
}